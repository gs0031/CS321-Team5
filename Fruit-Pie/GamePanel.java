    ...
    public void update() { 
        ...
        if (isDropping && fruitYRatio < 0.88f) {
            ...
            for (Float[] fruit : droppedFruits) {
                ...
                {
                    // Combine the two fruits by averaging their positions
                    fruitXRatio = (fruit[0] + fruitXRatio) / 2;  // Combine X positions
                    fruitYRatio = (fruit[1] + fruitYRatio) / 2;  // Combine Y positions

                    // Combine the colors (you could use a blending technique or choose one fruit's color)
                    //currentFruitColor = droppedFruitColors.get(i);  // Take the color of the collided fruit
        ...
        if (fruitYRatio < 0.88f) {
            ...
            if(keyH.validMouse)
            {
                ...
                fruitXRatio = keyH.xMouse/1500;
                System.out.println(keyH.xMouse);
                keyH.validMouse = false;
            }
            render();
        }
    }
